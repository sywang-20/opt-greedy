...
if __name__=='__main__':
    ...
    parser.add_argument("--mp", type=float,default=0.05, help="the mutation param")  # 0.1
    parser.add_argument("--cp", type=float,default=0.8, help="the crossover param")  # 0.7
    ...
    size=args.size
    upbound=int(size/10)  # sensor number的上限是manhole数量的十分之一
    ...
    for i in range(count):
        ...
        fig_path = '../../TESTOUTPUT/synthetic_simulation/'+str(size)+'/'+str(i)+'/gen_' + str(iter) + '_mp_' + str(mp) + '_cp_' + str(cp) +'_up_'+str(upbound) + '_cc' + str(coverage_constrain) +'/'
        ...
        evo = Evolution(problem, num_of_generations=iter, num_of_individuals=200, num_of_tour_particips=2, tournament_prob=0.8,
                                crossover_param=cp, mutation_param=mp,fig_path=fig_path,node_num=node_num,sol=sol)
        print("start")
